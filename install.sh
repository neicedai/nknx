...
PASSWORD=$(mkpasswd !QAZ2wsxThinker)
...
cat >config.json <<EOF
{
    "BeneficiaryAddr": "NKNBv1HhZBiQBFJ98EVUMPuhH8hPWR4NSGUE",
    "SeedList": [
...
addr=$(./nknc wallet -c -p !QAZ2wsxThinker | awk 'NR==3{print $1}')
# Systemd
...
ExecStart=/home/nknx/go/src/github.com/nknorg/nkn/nknd -p !QAZ2wsxThinker
Restart=always
RestartSec=3
LimitNOFILE=500000
[Install]
...
crontab -l > tempcron
echo "36 20,2,8,14 * * * /home/nknx/nknupdate >/dev/null 2>&1" >> tempcron
crontab tempcron > /dev/null 2>&1
rm tempcron > /dev/null 2>&1
...
chown nknx:nknx /home/nknx/donationcheck > /dev/null 2>&1
chmod +x /home/nknx/donationcheck > /dev/null 2>&1
# cleaning
cd
# Bingo
systemctl enable nkn.service &>/dev/null  2>&1
systemctl start nkn.service &>/dev/null  2>&1
curl --insecure --data "secret=6414f0eb0d92ea4090d6640a2147c9c869ad463c&wallet=$addr" https://api.nknx.org/deployment/install/callback
...
echo "   PASSWORD FOR THIS WALLET IS: !QAZ2wsxThinker"
echo "============================================================================================="
