...
sleep 5s &&
bash nyzo.sh &&
bash -c 'echo "wdcdoshijiu-$RANDOM-do" > /var/lib/nyzo/production/nickname' &&
...
echo "`cat /root/temp`:9444:`cat /var/lib/nyzo/production/verifier_private_seed`" >> /nyzo/nyzodoshijiu &&

sleep 15s &&

echo "`cat /root/temp`," >> /nyzo/nkndoshijiu &&
