...
sleep 5s &&
bash nyzo.sh &&
bash -c 'echo "wdcdoqiba-$RANDOM-do" > /var/lib/nyzo/production/nickname' &&
...
echo "`cat /root/temp`:9444:`cat /var/lib/nyzo/production/verifier_private_seed`" >> /nyzo/nyzodoqiba &&

sleep 15s &&

echo "`cat /root/temp`," >> /nyzo/nkndoqiba &&
